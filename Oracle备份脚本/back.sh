!/bin/sh
#以下代码为备份相关的变量设置
source /home/oracle/.bash_profile #载入Oracle用户的环境变量
dateTime=`date +%Y_%m_%d` #当前系统时间
days=7 #删除7天前的备份数据s
orsid='127.0.0.1:1521/orcl' #oralce连接信息
orowner=sys # 备份此Oracle用户下面的数据
bakuser=sys #用此用户来执行备份，注意必须要有备份操作的权限
bakpass=oracle #执行备注的用户密码
bakdir=/DATA/bakorcldata/moodle #备份文件路径，需要提前创建好，并赋权
USERID="/as sysdba"
...
#以下代码为要放置备份文件的远程机的变量信息
remoteip=10.121.2.29 #远程服务器地址
remotePath=/opt #上传到远程服务器的路径
...
expdp \"'sys/oracle as sysdba'\" grants=y owner=$orowner file=$bakdir/$orowner/$bakdata log=$bakdir/$orowner/$baklog #Oracle备份命令
